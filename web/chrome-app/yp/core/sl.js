sl.core = function(o) {
	...
	self.setMainEl = function(el) {
		if (el) {			
			sl.addEventListener(el,"mousemove",self.mouse,false);
			sl.addEventListener(el,"mouseup",self.mouse,false);
			sl.addEventListener(el,"mouseout",self.mouse,false);
			sl.addEventListener(el,"mouseover",self.mouse,false);
	...
	self.loadData = function(name,cb) {
		...
		var p = name.split("?");
		name = p.shift();
		sl.require((sl.config.isWeb?"":"../")+"data/"+name+".js"+(p.length?"?"+p:""),function(failed){
			if (data = sl.getDeepRef(sl.data,ref.join("."))) cb(data);
		});
	};
