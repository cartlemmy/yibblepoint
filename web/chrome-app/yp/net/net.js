sl.net = function(o) {
	...
	self.send = function(action,data,options,callback) {
		...
		if (self.httpQueueTimeout) { clearTimeout(self.httpQueueTimeout); }
		
		if (!options) options = {};
		
		if (options.queueTime === 0) options.queueTime = 5;
		...
		if (options.queueTime === 0 || sl.unixTS(true) > self.httpLastSend + self.pollFrequency || (options && options.blocking)) {
			self.httpSend();
		} else {
			self.httpQueueTimeout = setTimeout(self.httpSend,options.queueTime ? options.queueTime : 1000);
		}
	};
...
sl.XDomainRequest = function() {
	var self = this, req = new XDomainRequest();
	...
	self.open = function(method,url) {
		if (self.onerror) req.onerror = self.onerror;
		if (self.onabort) req.onabort = self.onabort;
		if (self.ontimeout) req.ontimeout = self.ontimeout;
		req.open(method,url);		
	};
	
	self.send = req.send;
	self.abort = req.abort;
