...
sl.isChildOf = function(el,parent) {
	...
	while (el.parentNode != parent) {
		if (el.slMenuButton && el.slMenuButton == parent) return true;
...
sl.reqMap = {"jquery":"../inc/thirdparty/jquery/jquery-2.2.3.min.js"};
...
sl.require = function(scripts,cb,loadAsData,params) {
	...
	for (var i = 0; i < scripts.length; i++) {
		...
		if (sl.reqMap[src]) scripts[i] = src = sl.reqMap[src];
		...
		if (!sl.scripts[src]) {
			...
			(function(src){
				...
				var el = null;
				var fullSrc = sl.getRootWWW(src)+src+(loadAsData?(src.indexOf("?") == -1 ? "?" : "&")+"lad=1":"");
...
sl.parseLink = function(link) {
	return sl.getRootWWW(link) + link;
};

sl.getRootWWW = function(path) {
	if (path.substr(0,1) == '/') return sl.config.webRoot;
	return sl.config.core.fromAPI || !sl.view ? (sl.config.root ? sl.config.root : sl.config.webRoot) : "";
}
